...
/**
 * Minimal host abstraction for .vscode/launch.json access.
 *
 * Why this exists:
 ...
 */
export interface LaunchJsonHost {
  /**
   * Read the raw launch.json text.
   *
  ...
  /**
   * Persist the full launch.json text.
   */
  writeText(text: string): Promise<void>;

  /**
   * Delete the launch.json file.
   *
 ...
 *
 * Important:
 * - this class does not try to discover the correct launch.json location
 * - it only reads/writes the path supplied by the caller
 */
export class FileLaunchJsonHost implements LaunchJsonHost {
